...
interface TouchTexture { canvas: HTMLCanvasElement; texture: THREE.Texture; addTouch: (norm: { x: number; y: number }) => void; update: () => void; radiusScale: number; size: number; }
...
type PixelBlastProps = {
  ...
  style?: React.CSSProperties; antialias?: boolean; patternScale?: number;
  patternDensity?: number; liquid?: boolean; liquidStrength?: number;
  liquidRadius?: number; pixelSizeJitter?: number; enableRipples?: boolean;
  rippleIntensityScale?: number; rippleThickness?: number; rippleSpeed?: number;
  liquidWobbleSpeed?: number; autoPauseOffscreen?: boolean; speed?: number;
  transparent?: boolean; edgeFade?: number; noiseAmount?: number;
};
...
const createTouchTexture = (): TouchTexture => {
  ...
  return { canvas, texture, addTouch, update,
    set radiusScale(v: number) { radius = 0.1 * size * v; },
    get radiusScale() { return radius / (0.1 * size); }, size };
};
...
const PixelBlast: React.FC<PixelBlastProps> = ({
  ...
  useEffect(() => {
    ...
    if (mustReinit) {
      ...
      const animate = () => {
        ...
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        if (liquidEffect) { const u = (liquidEffect as any).uniforms?.get('uTime'); if (u) u.value = uniforms.uTime.value; }
        if (composer) {
          if (touch) touch.update();
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          composer.passes.forEach((p: any) => { if (p.effects) p.effects.forEach((eff: any) => { const u = eff.uniforms?.get('uTime'); if (u) u.value = uniforms.uTime.value; }); });
          composer.render();
      ...
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      if (t.liquidEffect) { const u = (t.liquidEffect as any).uniforms?.get('uStrength'); if (u) u.value = liquidStrength; const f = (t.liquidEffect as any).uniforms?.get('uFreq'); if (f) f.value = liquidWobbleSpeed; }
  ...
  return <div ref={containerRef} style={{ width: '100%', height: '100%', position: 'relative', overflow: 'hidden', ...style }} aria-label="PixelBlast interactive background" />;
};
